/** @blazeo.com/calendar-client - type declarations */

export type AuthState = {
  accessToken?: string;
  tokenExpiresAt?: string;
};

export function configure(env: {
  baseUrl?: string;
  consumer?: string;
  fetch?: typeof fetch;
  getDefaultOffset?: () => number;
  accessToken?: string;
  expiresAtUtc?: string;
  tokenExpiresAt?: string;
  expires_at_utc?: string;
  /** Called when the stored JWT is missing or near expiry; return a fresh token from your IdP. */
  getAccessToken?: () => Promise<string | undefined>;
}): void;

export function getConfig(): {
  baseUrl?: string;
  consumer?: string;
  fetch?: typeof fetch;
  getDefaultOffset?: () => number;
  accessToken?: string;
  tokenExpiresAt?: string;
} | null;

export function setBaseUrl(baseUrl: string): void;
export function setConsumer(consumer: string): void;
export function setAccessToken(accessToken: string, expiresAtUtc?: string): void;
export function clearAccessToken(): void;
export function setGetAccessToken(fn: () => Promise<string | undefined>): void;
export function clearAuth(): void;
export function getAuth(): AuthState;
export function ensureValidAccessToken(): Promise<string | undefined>;
export function isAccessTokenExpired(expiresAtUtc?: string | number | Date | null, skewMs?: number): boolean;
export function buildAuthHeaders(extra?: Record<string, string>): Record<string, string>;
export const DEFAULT_TOKEN_REFRESH_SKEW_MS: number;
export function getConfigStore(): unknown;

export const ConfigModel: unknown;

export type ParticipantAuthorizationStatus = {
  participantId: string;
  isApproved: boolean;
};

export const CalendarModel: {
  get(calendarId: string): Promise<unknown>;
  getRaw(calendarId: string): Promise<{ status: string; data?: unknown; message?: string }>;
  getByCompany(
    companyKey: string,
    opts?: {
      skip?: number;
      take?: number;
      /** Maps to API query `sort`. */
      sortBy?: string;
      /** Maps to API query `sort_dir` (`asc` / `desc`). */
      sortOrder?: 'ASC' | 'DESC' | 'asc' | 'desc' | string;
      /** Same as `sortBy` (sent as `sort` on the wire). */
      sort?: string;
      sort_column?: string;
      /** Same as `sortOrder` (sent as `sort_dir` on the wire). */
      sort_dir?: 'asc' | 'desc' | string;
      page?: number;
      page_size?: number;
    }
  ): Promise<{ calendars: unknown[]; totalCount: number } | null>;
  getTimeZones(): Promise<unknown>;
  getTimeZone(timezoneId: string): Promise<unknown>;
  getParticipants(calendarId: string): Promise<unknown>;
  getAllParticipantOpeningHours(calendarId: string): Promise<unknown[] | null>;
  getCalendarParticipant(calendarId: string): Promise<unknown>;
  getParticipantsInfo(calendarId: string): Promise<unknown>;
  checkParticipantsAuthorization(calendarId: string): Promise<ParticipantAuthorizationStatus[] | null>;
  getMonth(calendarId: string, year: number, month: number): Promise<unknown>;
  getEvents(calendarId: string): Promise<unknown>;
  createWithParticipants(name: string, companyKey: string, participantIds: string[], description: string, calendarId?: string): Promise<unknown>;
  editWithParticipants(calendarId: string, name: string, participantIds: string[], description: string): Promise<unknown>;
  create(snapshot: object, options?: { env?: object }): unknown;
};

/** Event fields returned by the API (camelCase). */
export type EventSnapshot = {
  eventId: string;
  calendarId?: string;
  participantId?: string | null;
  title?: string | null;
  notes?: string | null;
  calendarLocationId?: string | null;
  /** From CalendarLocation.Value when event search endpoints enrich the row. */
  calendarLocationValue?: string | null;
  customLocation?: string | null;
  visitorName?: string | null;
  visitorEmail?: string | null;
  visitorPhone?: string | null;
  /** From the matching Lead.Source (by visitor email + company) when event get/search endpoints enrich the row. */
  leadSource?: string | null;
  startDate?: string;
  endDate?: string;
  [key: string]: unknown;
};

export type EventSearchResult = {
  events: unknown[];
  totalCount: number;
};

export type EventFlowData = {
  eventId: string;
  flowId: string | null;
  flowPath: string | null;
};

export type LeadCompanyResult = {
  leads: unknown[];
  totalCount: number;
};

export const EventModel: {
  get(eventId: string, opts?: { offset?: number }): Promise<unknown>;
  getFlow(eventId: string): Promise<EventFlowData | null>;
  getByExternalId(externalEventId: string, opts?: { offset?: number }): Promise<unknown>;
  getByDateRangeWithFilters(
    companyKey: string,
    startDateFrom: string,
    startDateTo: string,
    opts?: Record<string, unknown>
  ): Promise<EventSearchResult>;
  getUpcomingByDateRangeWithFilters(
    companyKey: string,
    startDateFrom: string,
    startDateTo: string,
    opts?: Record<string, unknown>
  ): Promise<EventSearchResult>;
  getByFilters(companyKey: string, opts?: Record<string, unknown>): Promise<EventSearchResult>;
  getRoundRobinParticipant(calendarId: string): Promise<unknown>;
  getEarliestAvailableDays(calendarId: string, count: number, opts?: { year?: number; month?: number; day?: number; offset?: number }): Promise<string[] | null>;
  getDaySelectable(calendarId: string, year: number, month: number, day: number, opts?: { participantId?: string; offset?: number }): Promise<boolean>;
  getByVisitorEmail(email: string, opts: { companyKey?: string; calendarId?: string; offset?: number }): Promise<unknown[] | null>;
  getByVisitorPhone(phone: string, opts: { companyKey?: string; calendarId?: string; offset?: number }): Promise<unknown[] | null>;
  getAvailability(calendarId: string, year: number, month: number, day: number, opts?: { participantId?: string; offset?: number }): Promise<unknown[]>;
  getExternalUnavailability(
    participantId: string,
    opts?:
      | { startUtc: string; endUtc: string; offset?: number }
      | { year: number; month: number; day: number; days?: number; offset?: number }
  ): Promise<unknown[]>;
  cancel(eventId: string, reason?: string): Promise<unknown>;
  delete(eventId: string): Promise<unknown>;
  getCancellable(eventId: string): Promise<boolean>;
  /** `optsOrOffset` may be a number (offset minutes, legacy) or `{ offset, source }`; `source` becomes the created lead's source. */
  createEvent(payload: object, optsOrOffset?: number | { offset?: number; source?: string }): Promise<unknown>;
  /** `optsOrOffset` may be a number (offset minutes) or `{ offset }`; throws on a non-numeric offset. */
  reschedule(payload: object, optsOrOffset?: number | { offset?: number }): Promise<unknown>;
  updateEvent(payload: object): Promise<unknown>;
  createTest(payload: object, offsetMinutes?: number): Promise<unknown>;
  getCustomData(calendarId: string, eventId?: string): Promise<unknown>;
  setReminder(eventId: string): Promise<unknown>;
  setAttendeeStatus(eventId: string, attendeeStatus: number | string): Promise<unknown>;
  setEventStatus(eventId: string, status: number | string): Promise<unknown>;
  create(snapshot: EventSnapshot | object, options?: { env?: object }): unknown;
};

export type ExternalEventSnapshot = {
  title?: string | null;
  startDate?: string;
  endDate?: string;
  startHour?: number;
  startMinute?: number;
  endHour?: number;
  endMinute?: number;
};

export const ExternalEventModel: {
  getByParticipant(participantId: string, opts?: { offset?: number }): Promise<unknown[]>;
  create(snapshot: ExternalEventSnapshot | object, options?: { env?: object }): unknown;
};

export const AvailabilityModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const AvailabilityDetailModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const CalendarParticipantModel: {
  getByCalendar(calendarId: string): Promise<unknown[] | null>;
  getInfoByCalendar(calendarId: string): Promise<unknown[] | null>;
  checkAuthorizationByCalendar(calendarId: string): Promise<ParticipantAuthorizationStatus[] | null>;
  getByParticipant(participantId: string): Promise<unknown[] | null>;
  create(snapshot: object, options?: { env?: object }): unknown;
};
export const ParticipantModel: {
  get(participantId: string): Promise<unknown>;
  getByEmail(email: string, companyKey: string): Promise<unknown>;
  getByIds(participantIds: string[] | string): Promise<unknown[] | null>;
  getAll(companyKey: string): Promise<unknown[] | null>;
  add(payload: object, calendarId?: string): Promise<unknown>;
  remove(participantId: string): Promise<unknown>;
  update(payload: object): Promise<unknown>;
  save(payload: object): Promise<unknown>;
  sendEmail(participantId: string): Promise<unknown>;
  migrate(participantId: string, userId: string): Promise<unknown>;
  create(snapshot: object, options?: { env?: object }): unknown;
};
export const OpeningHourModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const TimeSlotModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const TimeFrameModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const SettingModel: {
  get(calendarId: string): Promise<unknown>;
  save(payload: object): Promise<unknown>;
  uploadLogo(calendarId: string, file: File | Blob): Promise<unknown>;
  create(snapshot: object, options?: { env?: object }): unknown;
};
export type PhoneNumberPurchaseData = {
  formatted?: string;
  number?: string;
  phoneNumber?: string;
  phoneNumbers?: string;
  status?: number;
  Formatted?: string;
  Number?: string;
  PhoneNumber?: string;
  PhoneNumbers?: string;
  Status?: number;
};

export type PhoneNumberPurchaseResult = {
  data?: PhoneNumberPurchaseData;
  error?: string;
  isInnerException?: boolean;
  stackTrace?: string;
  success?: boolean;
  Data?: PhoneNumberPurchaseData;
  Error?: string;
  IsInnerException?: boolean;
  StackTrace?: string;
  Success?: boolean;
};

export type PhoneNumberReleaseData = {
  company_key?: string;
  phone_number?: string;
  released_from_twilio?: boolean;
  cleared_local_state?: boolean;
};

export const CompanyModel: {
  get(companyKey: string): Promise<unknown>;
  getAll(): Promise<unknown[] | null>;
  save(payload: object): Promise<unknown>;
  purchasePhoneNumber(companyKey: string, companyId: string): Promise<{
    status: string;
    data?: PhoneNumberPurchaseResult;
    message?: string;
  }>;
  releasePhoneNumber(companyKey: string, phoneNumber?: string): Promise<{
    status: string;
    data?: PhoneNumberReleaseData;
    message?: string;
  }>;
  create(snapshot: object, options?: { env?: object }): unknown;
};
export const AssetModel: {
  upload(
    file: File | Blob,
    opts: {
      category: string;
      companyKey?: string;
      calendarId?: string;
      consumer?: string;
    }
  ): Promise<{ status: string; data?: unknown; message?: string }>;
  create(
    snapshot?: {
      url?: string | null;
      blobPath?: string | null;
      category?: string | null;
      contentType?: string | null;
      size?: number | null;
    },
    options?: { env?: object }
  ): unknown;
};
export const CalendarDayModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const ParticipantInfoModel: { create(snapshot: object, options?: { env?: object }): unknown };
export const CalendarLocationModel: {
  getByCalendar(calendarId: string): Promise<unknown[] | null>;
  save(payload: object): Promise<unknown>;
  remove(calendarLocationId: string): Promise<unknown>;
  setDefault(calendarId: string, calendarLocationId: string): Promise<unknown>;
  create(
    snapshot?: {
      id?: number | null;
      calendarLocationId?: string | null;
      calendarId?: string | null;
      locationType?: number;
      name?: string;
      value?: string;
      isDefault?: boolean;
      sortOrder?: number;
      createdOn?: string | null;
      modifiedOn?: string | null;
    },
    options?: { env?: object }
  ): unknown;
};
export const PreferenceModel: {
  getScopes(): Promise<string[] | null>;
  getOptions(): Promise<string[] | null>;
  getOptionTemplate(option: string): Promise<unknown>;
  get(option: string, keys: string | string[]): Promise<unknown>;
  set(scope: string, key: string, option: string, body: string | object): Promise<unknown>;
  delete(preferenceId: string): Promise<unknown>;
  create(snapshot: object, options?: { env?: object }): unknown;
};
export const PreferenceScope: Record<string, number>;

export type FlowSnapshot = {
  id?: number | null;
  flowId?: string;
  companyKey?: string;
  calendarId?: string | null;
  name?: string | null;
  description?: string | null;
  flowJson?: string;
  isActive?: boolean;
  isDeleted?: boolean;
  createdOn?: string | null;
  modifiedOn?: string | null;
};

export type FlowListSortKey =
  | 'id'
  | 'flowId'
  | 'calendarId'
  | 'name'
  | 'description'
  | 'isActive'
  | 'createdOn'
  | 'modifiedOn';

export type FlowListOptions = {
  /** Sort key. Aliases: sort, sort_column. */
  sortBy?: FlowListSortKey | string;
  sort?: FlowListSortKey | string;
  sort_column?: FlowListSortKey | string;
  /** Sort direction. Aliases: sort_dir. Defaults to createdOn descending when omitted. */
  sortOrder?: 'asc' | 'desc' | string;
  sort_dir?: 'asc' | 'desc' | string;
};

export const FlowModel: {
  getRaw(flowId: string): Promise<{ status: string; data?: unknown; message?: string }>;
  list(companyKey: string, includeDeleted?: boolean, opts?: FlowListOptions): Promise<unknown[] | null>;
  get(flowId: string): Promise<unknown>;
  createFlow(payload: FlowSnapshot): Promise<unknown>;
  updateFlow(payload: FlowSnapshot): Promise<unknown>;
  delete(flowId: string): Promise<unknown>;
  duplicate(flowId: string, newName?: string): Promise<unknown>;
  getAppearance(flowId: string): Promise<unknown>;
  saveAppearance(payload: object): Promise<unknown>;
  getEmbed(flowId: string): Promise<unknown>;
  saveEmbed(payload: object): Promise<unknown>;
  getPublic(flowId: string): Promise<{ flow: unknown; appearance: unknown; embed: unknown } | null>;
  getPreview(flowId: string): Promise<{ flow: unknown; appearance: unknown; embed: unknown } | null>;
  /** Empty snapshot is valid: `FlowModel.create({}, { env })` after configure() */
  create(snapshot?: FlowSnapshot, options?: { env?: object }): unknown;
};

export type ActivityLogSearchResult = {
  activityLogs: unknown[];
  totalCount: number;
};

export type ActivityLogSnapshot = {
  id?: number | null;
  activityLogId?: string;
  leadId?: string | null;
  eventId?: string | null;
  activityTypeId?: number;
  activityTypeName?: string;
  categoryId?: number;
  categoryName?: string;
  isPartialData?: boolean;
  userEmail?: string;
  createdOn?: string | null;
  appointmentId?: number | null;
  title?: string;
  startDate?: string | null;
  endDate?: string | null;
  timeZone?: string;
  source?: string;
  meetingWith?: string;
  handledBy?: string;
  location?: string;
  summary?: string;
  transcript?: string;
  userDefinedFields?: Record<string, unknown> | null;
  newStartDate?: string | null;
  newEndDate?: string | null;
  oldStartDate?: string | null;
  oldEndDate?: string | null;
};

export const ActivityLogModel: {
  get(opts: {
    companyKey: string;
    company_key?: string;
    leadId?: string;
    lead_id?: string;
    eventId?: string;
    event_id?: string;
    activityTypeId?: number | string | number[];
    activity_type_id?: number | string | number[];
    activityTypeIds?: number | string | number[];
    userEmail?: string;
    user_email?: string;
    fromDate?: string;
    from_date?: string;
    toDate?: string;
    to_date?: string;
    sortBy?: string;
    sort?: string;
    sort_column?: string;
    sortOrder?: 'ASC' | 'DESC' | 'asc' | 'desc' | string;
    sort_dir?: 'asc' | 'desc' | string;
    searchColumn?: string;
    search_column?: string;
    column?: string;
    searchText?: string;
    search_text?: string;
    search?: string;
    filters?: Array<{
      column?: string;
      searchColumn?: string;
      search_column?: string;
      text?: string;
      searchText?: string;
      search_text?: string;
      search?: string;
    }>;
    page?: number;
    pageSize?: number;
    page_size?: number;
    skip?: number;
    take?: number;
    offset?: number;
  }): Promise<ActivityLogSearchResult | { status: 'failure'; message: string } | null>;
  getByCompany(
    companyKey: string,
    opts?: Omit<Parameters<typeof ActivityLogModel.get>[0], 'companyKey'>
  ): Promise<ActivityLogSearchResult | { status: 'failure'; message: string } | null>;
  create(snapshot?: ActivityLogSnapshot, options?: { env?: object }): unknown;
};

export const LeadModel: {
  getRaw(leadId: string): Promise<{ status: string; data?: unknown; message?: string }>;
  /** `is_new` and the recorded view are derived from the authenticated user's email (JWT); optional `{ userId }` only selects that user's saved columns. */
  get(leadId: string, opts?: { userId?: string; user_id?: string }): Promise<unknown>;
  getByEmail(email: string, companyKey: string): Promise<unknown>;
  getByPhone(phone: string, companyKey: string): Promise<unknown>;
  allowedColumns: string[];
  saveColumnSelection(
    userId: string,
    columns: string[]
  ): Promise<{ status: string; data?: unknown; message?: string }>;
  getColumnSelection(userId: string): Promise<{
    status: string;
    data?: { userId?: string; columns?: string[]; isDefault?: boolean };
    message?: string;
  }>;
  /** Basic lead columns a calendar's booking form can ask for (matches API BookableColumnKeys). */
  bookableColumns: string[];
  /** GET /lead/fields/get – calendar booking-form basic field config (asked + required). */
  getFieldRequirements(calendarId: string): Promise<{
    status: string;
    data?: {
      calendarId?: string;
      fields?: Array<{ column: string; enabled: boolean; required: boolean }>;
      isDefault?: boolean;
      available?: string[];
    };
    message?: string;
  }>;
  /** POST /lead/fields/save – save which basic lead fields the calendar asks for / requires. */
  saveFieldRequirements(
    calendarId: string,
    fields: Array<string | { column: string; enabled?: boolean; required?: boolean }>
  ): Promise<{
    status: string;
    data?: {
      calendarId?: string;
      fields?: Array<{ column: string; enabled: boolean; required: boolean }>;
    };
    message?: string;
  }>;
  /** Requires firstName/name, leadType, companyKey and at least one of email or phone. */
  createLead(payload: {
    companyKey: string;
    email?: string;
    phone?: string;
    /** Legacy alias for firstName. */
    name?: string;
    firstName?: string;
    first_name?: string;
    lastName?: string;
    last_name?: string;
    salutation?: string;
    source?: string;
    leadType: 'sales' | 'service' | 'others' | 'none' | number;
    referrerLink?: string;
    description?: string;
  }): Promise<{ status: string; data?: unknown; message?: string } | null>;
  /** Requires leadId, firstName/name and leadType; when email or phone is included, at least one must be non-empty. */
  updateLead(payload: {
    leadId: string;
    email?: string;
    companyKey?: string;
    /** Legacy alias for firstName. */
    name?: string;
    firstName?: string;
    first_name?: string;
    lastName?: string;
    last_name?: string;
    salutation?: string;
    phone?: string;
    source?: string;
    leadType: 'sales' | 'service' | 'others' | 'none' | number;
    referrerLink?: string;
    description?: string;
  }): Promise<unknown>;
  deleteLead(leadId: string): Promise<{ status: string; data?: unknown; message?: string }>;
  /**
   * GET /lead/sources/allowed – lead source values the calling consumer may set (its own system
   * source plus 'none' / 'Other'). The Consumer header is attached from configure({ consumer });
   * pass opts.consumer to override it for this call.
   */
  getAllowedSources(opts?: { consumer?: string; Consumer?: string }): Promise<string[]>;
  requestExport(
    companyKey: string,
    opts?: {
      notifyPath?: string;
      notify_path?: string;
      consumer?: string;
      Consumer?: string;
      /** Recipient for lead-export completion notification (or omit to use JWT email claim). */
      userEmail?: string;
      user_email?: string;
    }
  ): Promise<{ status: string; data?: unknown; message?: string }>;
  getByCompany(
    companyKey: string,
    opts?: {
      userId?: string;
      user_id?: string;
      skip?: number;
      take?: number;
      /** Maps to API query `sort`. */
      sortBy?: string;
      /** Maps to API query `sort_dir` (`asc` / `desc`). */
      sortOrder?: 'ASC' | 'DESC' | 'asc' | 'desc' | string;
      /** Same as `sortBy` (sent as `sort` on the wire). */
      sort?: string;
      sort_column?: string;
      /** Same as `sortOrder` (sent as `sort_dir` on the wire). */
      sort_dir?: 'asc' | 'desc' | string;
      searchColumn?: string;
      search_column?: string;
      column?: string;
      searchText?: string;
      search_text?: string;
      search?: string;
      /** Multiple column filters, ANDed together (combined with the single searchColumn/searchText pair when both are set). */
      filters?: Array<{
        column?: string;
        searchColumn?: string;
        search_column?: string;
        text?: string;
        searchText?: string;
        search_text?: string;
        search?: string;
      }>;
      page?: number;
      page_size?: number;
    }
  ): Promise<LeadCompanyResult | null>;
  /** Empty snapshot is valid: `LeadModel.create({}, { env })` after configure() */
  create(snapshot?: object, options?: { env?: object }): unknown;
};

export const AuthorizationSource: {
  None: 0;
  Direct: 1;
  Email: 2;
};

export type ParticipantCredentialsData = {
  participantCredentialId?: string;
  participantId: string;
  email?: string;
  authorizedByParticipantId?: string | null;
  authorizationSource?: number | null;
  authorizedOn?: string | null;
};

export type CalendarProviderOption = {
  emailProvider: number;
  key: string;
  name: string;
  displayName: string;
  description: string;
  scope: string;
  redirectUrl: string;
};

export type ParticipantAuthorizationUrl = {
  participantId: string;
  emailProvider: number;
  providerKey: string;
  authorizationUrl: string;
  redirectUrl: string;
};

export type CalendarProviderAuthorizationState = {
  emailProvider: number;
  key: string;
  name: string;
  hasCredentials: boolean;
  isAuthorized: boolean;
  isSelected: boolean;
};

export type ParticipantAuthorizationStatus = {
  participantId: string;
  email: string;
  emailProvider: number;
  providerKey: string;
  hasCredentials: boolean;
  isAuthorized: boolean;
  providers: CalendarProviderAuthorizationState[];
};

export type ParticipantData = {
  id?: number | null;
  participantId: string;
  companyKey?: string | null;
  alias?: string;
  email?: string;
  isApproved: boolean;
  isAvailable: boolean;
  provider: number;
  createdOn?: string | null;
  modifiedOn?: string | null;
  isDeleted?: boolean;
  /** Last stored authorization error from ParticipantCredentials. */
  authorizationError?: string;
  /** Participant who initiated calendar authorization (in-app flow). */
  authorizedByParticipantId?: string | null;
  /** 1 = Direct, 2 = Email — from ParticipantCredentials. */
  authorizationSource?: number | null;
  /** When calendar OAuth last completed successfully. */
  authorizedOn?: string | null;
};

export const CALENDAR_AUTH_MESSAGE_TYPE: 'calendar-authorization';

export const CalendarEmailProvider: {
  Google: 1;
  Microsoft: 2;
  Default: 3;
};

export const AuthModel: {
  CALENDAR_AUTH_MESSAGE_TYPE: typeof CALENDAR_AUTH_MESSAGE_TYPE;
  EmailProvider: typeof CalendarEmailProvider;
  getCalendarProviders(opts?: { host?: string }): Promise<{
    status: string;
    data?: CalendarProviderOption[];
    message?: string;
  }>;
  getAuthorizationUrl(
    participantId: string,
    emailProvider: string | number,
    opts?: { host?: string; authorizedByParticipantId?: string }
  ): Promise<{
    status: string;
    data?: ParticipantAuthorizationUrl;
    message?: string;
  }>;
  getAuthorizationStatus(participantId: string): Promise<{
    status: string;
    data?: ParticipantAuthorizationStatus;
    message?: string;
  }>;
  IsAuthorized(participantId: string): Promise<{
    status: string;
    data?: ParticipantData;
    message?: string;
  }>;
  openOAuthPopup(
    authorizationUrl: string,
    opts?: { width?: number; height?: number }
  ): Window | null;
  onCalendarAuthMessage(
    handler: (payload: {
      type: string;
      status: 'success' | 'error';
      message?: string;
      closePopup?: boolean;
    }) => void,
    popup?: Window | null
  ): () => void;
};

export const CustomFieldModel: {
  getAll(calendarId: string): Promise<unknown[] | null>;
  getFieldType(fieldType: string): Promise<unknown>;
  getFieldTypes(): Promise<unknown>;
  add(payload: object): Promise<unknown>;
  removeField(customFieldId: string): Promise<unknown>;
  removeAllFields(calendarId: string): Promise<unknown>;
  getForm(calendarId: string, dataId?: string): Promise<unknown>;
  saveForm(calendarId: string, fields: unknown[]): Promise<unknown>;
  getFormData(calendarId: string, dataId?: string): Promise<unknown>;
  saveFormData(dataId: string, fields: unknown[]): Promise<unknown>;
  saveFormFieldData(fieldPayload: object): Promise<unknown>;
  create(snapshot?: object, options?: { env?: object }): unknown;
};

export const RootStore: unknown;
export function createRootStore(initialState?: object): unknown;

export const Unit: Record<string, number>;
/**
 * Allowed lead-source values for event/lead creation/update (Lead.Source). Each value except
 * `None`/`Other` is a "system" identified by the `Consumer` request header; a consumer may only set
 * its own system source, `None`, or `Other`. Use `LeadModel.getAllowedSources()` for the subset a
 * given consumer is allowed to use.
 */
export const LeadSource: {
  BookingPage: 'Booking Page';
  Chatbot: 'Chatbot';
  CrmFlow: 'crmflow';
  LiveChat: 'Live Chat';
  SmartHub: 'SmartHub';
  VoiceAI: 'Voice AI';
  None: 'none';
  Other: 'Other';
};
/** Canonical list of all allowed lead-source values (includes the universal 'none' / 'Other'). */
export const LEAD_SOURCES: string[];
export const AssignmentMethod: Record<string, number>;
export const AttendeeStatus: Record<string, number>;
export const EventStatus: Record<string, number>;
export const RecurringFrequency: Record<string, number>;
/**
 * Reminder recipient (Models.Appointment.PreferenceOptions.Recipients).
 * Use when building the `Recipient` list of Email/SMS/InApp EventReminder options.
 */
export const Recipients: {
  Lead: 0;
  Agent: 1;
  LeadAndAgent: 2;
};
/** Lead type (Models.Appointment.LeadType). */
export const LeadType: {
  Sales: 0;
  Service: 1;
  Others: 2;
  None: 3;
};
export const DayOfWeek: Record<string, number>;
/** Custom field kind (Models.Appointment.CustomFieldKind). */
export const CustomFieldKind: {
  Unspecified: 0;
  Question: 1;
  LeadField: 2;
};
export const LocationType: Record<string, number>;
export const EmailProvider: Record<string, number>;
export const CalendarVisibility: Record<string, number>;
export const ActivityLogType: {
  AppointmentScheduled: 9;
  AppointmentRescheduled: 10;
  AppointmentCanceled: 11;
};
export const ActivityLogCategory: {
  Appointments: 3;
};
