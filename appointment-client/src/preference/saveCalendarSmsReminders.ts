...
export async function saveCalendarSmsRemindersPreference(
  ...
  | { ok: true; skipped: false; payload: ReturnType<typeof mapSmsRemindersToPreferencePayload>; response: unknown }
  | { ok: false; error: string; detail?: string; apiResponse?: unknown }
> {
  const id = String(calendarId ?? "").trim();
  if (!id) {
    return { ok: false, error: "calendarId is required to save SMS reminders preference." };
  }

  const payload = mapSmsRemindersToPreferencePayload(collectAppointmentReminders(calendar));
  if (payload.length === 0) {
  ...
  const result = await setPreferenceAsync(scope, id, SMS_EVENT_REMINDER_OPTION, payload, connection);
  if (!result.ok) {
  ...
  if (isFailureStatus(result.response)) {
    ...
    return {
      ok: false,
      error: msg || "SMS reminders preference save failed",
      apiResponse: res,
    };
  }
